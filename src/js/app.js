...
class MultiGuiasApp {
  ...
  initElements() {
    ...
    // Listeners do Electron para BrowserView em modo desktop
    if (this.isElectron) {
      window.electronAPI.onUrlChanged(({ paneId, url }) => {
      ...
      window.electronAPI.onTitleChanged(({ paneId, title }) => {
        const pane = this.panes.find(p => p.id === paneId);
        if (pane && title) {
          pane.titleEl.textContent = title.length > 25 ? title.substring(0, 25) + '...' : title;
        }
      });

      window.electronAPI.onLoadingChanged(({ paneId, loading }) => {
        const pane = this.panes.find(p => p.id === paneId);
        if (pane) {
          if (loading) pane.progressBar.classList.add('loading');
          else pane.progressBar.classList.remove('loading');
        }
      });

      // ResizeObserver para manter as BrowserViews perfeitamente alinhadas aos painéis
      ...
      window.addEventListener('resize', () => this.syncPaneBounds());
    }
  ...
  syncPaneBounds() {
    ...
    this.panes.forEach(pane => {
      ...
      const isVisible = pane.element.offsetParent !== null && pane.currentUrl !== '';
      boundsMap[pane.id] = {
        x: rect.left,
        y: rect.top,
        width: rect.width,
  ...
  bindPaneEvents(pane) {
    ...
    paneEl.querySelector('.btn-nav-back').addEventListener('click', () => {
      if (this.isElectron && window.electronAPI?.goBack) {
        window.electronAPI.goBack(pane.id);
    ...
    paneEl.querySelector('.btn-nav-forward').addEventListener('click', () => {
      if (this.isElectron && window.electronAPI?.goForward) {
        window.electronAPI.goForward(pane.id);
  ...
  loadUrlInIframe(pane, url, pushHistory = true) {
    ...
    // Oculta speed dial e exibe iframe / view
    ...
    if (this.isElectron && window.electronAPI?.navigate) {
      pane.iframeEl.style.display = 'none';
      window.electronAPI.navigate(pane.id, url);
      setTimeout(() => this.syncPaneBounds(), 50);
    } else {
  ...
  reloadPane(pane) {
    if (pane.currentUrl) {
      ...
      if (this.isElectron && window.electronAPI?.reload) {
        window.electronAPI.reload(pane.id);
      } else {
